...
class SearchController < ApplicationController
  ...
  def index
    ...
    respond_to do |format|
      ...
      format.xls  { send_data @search_results.to_xls, :type => :xls }
      format.csv  { send_data @search_results.to_csv, :type => :csv }
  ...
  def get_search_results(options = {})
    ...
    test = [:name,:tag]
    orderby = :created_at
    @keywordtext = params[:kw_text]
    @startdatatext = params[:start_date]
    @enddatetext = params[:end_date]
    
    search = Umtxt.search do
      fulltext params[:full_text].to_s + " " + params[:kw_text].to_s , :fields => test# do
       # fields(:title)
       #with :testid, [1,111]
      #end
