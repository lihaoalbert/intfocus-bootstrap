...
module IntfocusCore
  module Callback
    @@classes   = []  # Classes that inherit from IntfocusCore::Callback::Base.
    ...
    # Adds a class inherited from from IntfocusCore::Callback::Base.
    #--------------------------------------------------------------------------
    def self.add(klass)
      @@classes << klass
    end
    ...
    class Base
      ...
      def self.inherited(child)
        IntfocusCore::Callback.add(child)
    ...
    # This makes it possible to call hook() without IntfocusCore::Callback prefix.
    ...
    module Helper
      def hook(method, caller, context = {}, &block)
        ...
        if block_given? and is_view_hook
          hooks = IntfocusCore::Callback.view_hook(method, caller, context)
          ...
          data = IntfocusCore::Callback.hook(method, caller, context)
...
end # module IntfocusCore


ActionView::Base.send(:include, IntfocusCore::Callback::Helper)
ActionController::Base.send(:include, IntfocusCore::Callback::Helper)
