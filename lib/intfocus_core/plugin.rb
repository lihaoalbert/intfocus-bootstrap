...
Rails::Plugin.class_eval do
  def initializer
    ...
    if Rails.env.development?
      config.cache_classes = false # Tell Rails not to reload core classes when developing Intfocus Project plugin.
    end
    self
  end
end
  ...
  class Plugin
    ...
    def tab(main_or_admin, options = nil)
      ...
      if Setting.database_and_table_exists?
        tabs = IntfocusCore::Tabs.send(main_or_admin)
        if tabs                         # Might be nil when running rake task (ex: rake rfocus:setup).
          if block_given?
            yield tabs
          else
