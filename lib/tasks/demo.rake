...
namespace :rfocus do
  namespace :demo do
    ...
    task :load => :environment do
      ...
      Dir.glob(IntfocusCore.root.join('db', 'demo', '*.{yml,csv}')).each do |fixture_file|
        ActiveRecord::Fixtures.create_fixtures(IntfocusCore.root.join('db/demo'), File.basename(fixture_file, '.*'))
      end
    ...
    task :reload => :environment do
      ...
      Rake::Task["rfocus:demo:load"].invoke
    end
  end
end
